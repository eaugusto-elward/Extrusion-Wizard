...
#include <Standard_Version.hxx>  // For OCC_VERSION_STRING_EXT
	...
	initWindowHeight(480)     // Initialize the initial window height
{
	...
	myViewer->SetDefaultBackgroundColor(Quantity_NOC_WHITE); // Set the default background color to black
	...
	myViewCube->SetTextColor(Quantity_NOC_BLACK);           // Set the text color of the view cube
...
void OcctQtViewer::AddZAxisZeroLine(const Handle(AIS_InteractiveContext)& theContext)
{
	...
	gp_Pnt aPnt2(0.0, 0.0, 100.0);  // End point 
...
{
	if (!myXAxisZeroLine.IsNull())
	{
		if (myContext->IsDisplayed(myXAxisZeroLine))
		{
			myContext->Erase(myXAxisZeroLine, Standard_True); // Hide the line
		}
		else
		{
			myContext->Display(myXAxisZeroLine, Standard_True); // Show the line
		}
		update();
...
void OcctQtViewer::toggleFPS()
{
	if(myView->ChangeRenderingParams().ToShowStats == true)
		{
...
void OcctQtViewer::createBasicGrid()
{
	myViewer->ActivateGrid(Aspect_GT_Rectangular, Aspect_GDM_Lines); // Activate a rectangular grid

	// Create the grid and configure it
	Handle(Aspect_RectangularGrid) grid = Handle(Aspect_RectangularGrid)::DownCast(myViewer->Grid());
...
void OcctQtViewer::openGlInfoBox()
{
	...
	QMessageBox::information(0, "About Wizard", QString()
		+ "OCCT 3D Viewer sample embedded into Qt Widgets.\n\n"
		+ "Open CASCADE Technology v." OCC_VERSION_STRING_EXT "\n"
		+ "Qt v." QT_VERSION_STR "\n\n"
		+ "OpenGL info:\n"
